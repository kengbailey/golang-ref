...
func main() {
	...
	// The profile struct above has some extra information attached to each field.
	// This helps translate json to golang struct

	jsonText := []byte(`{"id":23, "name": "kenneth", "age": 26, "dob":"aug 9 1992"}`)
	var prof profile
	err := json.Unmarshal(jsonText, &prof)
	if err != nil {
		log.Fatal(err)
	}
